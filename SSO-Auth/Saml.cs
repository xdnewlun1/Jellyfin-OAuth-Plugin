...
public class Response
{
    ...
    /// <summary>
    /// Checks whether the XML response is valid by verifying the signature,
    /// and optionally validates audience restriction and recipient (SSO-14).
    /// </summary>
    /// <param name="expectedAudience">The expected audience (SAML client ID). Null to skip check.</param>
    /// <param name="expectedRecipient">The expected recipient (ACS URL). Null to skip check.</param>
    /// <returns>Whether the XML response is valid.</returns>
    public bool IsValid(string expectedAudience, string expectedRecipient)
    {
        ...
        if (!ValidateSignatureReference(signedXml) || !signedXml.CheckSignature(_certificate, true) || IsExpired())
        {
            return false;
        }

        // Validate NotBefore condition (SSO-14).
        var conditionsNode = _xmlDoc.SelectSingleNode(
            "/samlp:Response/saml:Assertion[1]/saml:Conditions", _xmlNameSpaceManager);
        if (conditionsNode?.Attributes["NotBefore"] != null)
        {
            if (DateTime.TryParse(conditionsNode.Attributes["NotBefore"].Value, out var notBefore))
            {
                if (DateTime.UtcNow < notBefore.ToUniversalTime())
                {
                    return false;
                }
            }
        ...
        // Validate Audience Restriction (SSO-14).
        if (!string.IsNullOrEmpty(expectedAudience))
        {
            var audienceNode = _xmlDoc.SelectSingleNode(
                "/samlp:Response/saml:Assertion[1]/saml:Conditions/saml:AudienceRestriction/saml:Audience",
        ...
        // Validate Recipient (SSO-14).
        if (!string.IsNullOrEmpty(expectedRecipient))
        {
            var recipientNode = _xmlDoc.SelectSingleNode(
                "/samlp:Response/saml:Assertion[1]/saml:Subject/saml:SubjectConfirmation/saml:SubjectConfirmationData",
                _xmlNameSpaceManager);
            if (recipientNode?.Attributes["Recipient"] != null
                && recipientNode.Attributes["Recipient"].Value != expectedRecipient)
            {
                return false;
            }
        }
    ...
    /// </summary>
    /// <param name="attr">The custom attribute to query. Must not contain single quotes (SSO-19).</param>
    /// <returns>The custom attribute.</returns>
    internal string GetCustomAttribute(string attr)
    {
        if (attr.Contains('\''))
        {
            throw new ArgumentException("Attribute name must not contain single quotes", nameof(attr));
        }
    ...
    internal List<string> GetCustomAttributes(string attr)
    {
        if (attr.Contains('\''))
        {
            throw new ArgumentException("Attribute name must not contain single quotes", nameof(attr));
