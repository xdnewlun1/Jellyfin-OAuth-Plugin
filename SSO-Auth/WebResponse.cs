...
";
    ...
    public static string Generator(string data, string provider, string baseUrl, string mode, bool isLinking = false)
    {
        ...
        // Serialize all dynamic values as JSON to avoid XSS via string interpolation into JS.
        // System.Text.Json escapes HTML-sensitive characters (<, >, &, ') as \uXXXX sequences,
        // making this safe to embed in a <script type="application/json"> element.
...
<script type=""application/json"" id=""sso-config"">" + configJson + @"</script>
<script>
const ssoConfig = JSON.parse(document.getElementById('sso-config').textContent);
...
async function link(request) {
    ...
    const url = ssoConfig.baseUrl + '/sso/' + ssoConfig.mode + '/Link/' + ssoConfig.provider + '/' + jfUser;
...
async function main() {
    ...
    document.getElementById('iframe-main').src = ssoConfig.baseUrl + '/web/index.html';

    var data = ssoConfig.data;
    ...
    if (ssoConfig.isLinking) await link(request);

    var url = ssoConfig.baseUrl + '/sso/' + ssoConfig.mode + '/Auth/' + ssoConfig.provider;
    ...
    window.location.replace(ssoConfig.baseUrl + '/web/index.html');
}
