...
public class SSOController : ControllerBase
{
        ...
        [FromQuery] string state) // Although this is a GET function, this function is called `Post` for consistency with SAML
    {
        ...
        if (config.Enabled)
        {
            ...
            // Validate the nonce cookie to ensure only the client that initiated
            // the flow can complete it (SSO-08).
            if (!Request.Cookies.TryGetValue("sso_state_nonce", out var cookieNonce)
                || cookieNonce != timedState.Nonce)
            {
                return BadRequest("State validation failed");
            }

            Response.Cookies.Delete("sso_state_nonce", new CookieOptions { Path = "/sso/" });
            ...
            if (result.IsError)
            {
                _logger.LogWarning("OIDC login error for provider {Provider}: {Error} - {Description}", provider, result.Error, result.ErrorDescription);
                return ReturnError(StatusCodes.Status400BadRequest, "Error logging in. Please try again or contact an administrator.");
            }
    ...
    public async Task<ActionResult> OidChallenge(string provider, [FromQuery] bool isLinking = false)
    {
        ...
        if (config.Enabled)
        {
            ...
            if (state.IsError)
            {
                _logger.LogWarning("OIDC prepare login error for provider {Provider}: {Error} - {Description}", provider, state.Error, state.ErrorDescription);
                return ReturnError(StatusCodes.Status400BadRequest, "Error preparing login. Please try again or contact an administrator.");
            }

            var timedState = new TimedAuthorizeState(state, DateTime.UtcNow);
            timedState.IsLinking = isLinking;

            // Generate a nonce and bind it to the state so that only the client
            // that initiated the flow can complete it (SSO-08).
            var nonce = Convert.ToBase64String(RandomNumberGenerator.GetBytes(32));
            timedState.Nonce = nonce;

            StateManager.TryAdd(state.State, timedState);

            Response.Cookies.Append("sso_state_nonce", nonce, new CookieOptions
            {
                HttpOnly = true,
                Secure = true,
    ...
    {
        // Redact sensitive secrets before returning (SSO-11).
        // Serialize to JSON, replace OidSecret values, and return the redacted JSON.
        var configs = SSOPlugin.Instance.Configuration.OidConfigs;
        var redacted = configs.ToDictionary(
            kvp => kvp.Key,
            kvp =>
            {
                var json = Newtonsoft.Json.JsonConvert.SerializeObject(kvp.Value);
                var obj = JObject.Parse(json);
                if (obj["OidSecret"] != null && !string.IsNullOrEmpty(obj["OidSecret"].ToString()))
                {
                    obj["OidSecret"] = "***";
                }
    ...
    {
        // Return only non-sensitive metadata; do not expose state secrets or tokens (SSO-11).
        var summary = StateManager.Select(kvp => new
        {
            kvp.Value.Created,
            kvp.Value.Valid,
            kvp.Value.Username,
    ...
    public ActionResult SamlPost(string provider, [FromQuery] string relayState = null)
    {
        ...
        _logger.LogInformation(
            "SAML request has relayState of {RelayState}", relayState);

        if (config.Enabled)
        {
            var samlResponse = new Response(config.SamlCertificate, Request.Form["SAMLResponse"]);

            // Validate signature, audience, and recipient (SSO-14).
            string samlAcsUrl = GetRequestBase(config.SchemeOverride, config.PortOverride)
                + $"/sso/SAML/{(config.NewPath ? "post" : "p")}/" + provider;
            if (!samlResponse.IsValid(config.SamlClientId?.Trim(), samlAcsUrl))
            {
                return Problem("Invalid SAML response");
            }
    ...
    {
        // Redact sensitive secrets before returning (SSO-11).
        var configs = SSOPlugin.Instance.Configuration.SamlConfigs;
        var redacted = configs.ToDictionary(
            kvp => kvp.Key,
            kvp =>
            {
                var json = Newtonsoft.Json.JsonConvert.SerializeObject(kvp.Value);
                var obj = JObject.Parse(json);
                if (obj["SamlCertificate"] != null && !string.IsNullOrEmpty(obj["SamlCertificate"].ToString()))
                {
                    obj["SamlCertificate"] = "***";
                }
    ...
    public async Task<ActionResult> SamlAuth(string provider, [FromBody] AuthResponse response)
    {
        ...
        if (config.Enabled)
        {
            ...
            // Validate signature and audience; recipient was already checked in SamlPost (SSO-14).
            if (!samlResponse.IsValid(config.SamlClientId?.Trim(), null))
            {
                return Problem("Invalid SAML response");
            }
    ...
    private async Task<Guid> CreateCanonicalLinkAndUserIfNotExist(string mode, string provider, string canonicalName)
    {
        ...
        if (user == null)
        {
            _logger.LogInformation("SSO user {CanonicalName} doesn't exist, creating...", canonicalName);
    ...
    private ActionResult SamlLink(string provider, Guid jellyfinUserId, AuthResponse response)
    {
        ...
        // Validate signature and audience (SSO-14).
        if (!samlResponse.IsValid(config.SamlClientId?.Trim(), null))
        {
            return Problem("Invalid SAML response");
        }
    ...
    private async Task<AuthenticationResult> Authenticate(Guid userId, bool isAdmin, bool enableAuthorization, bool enableAllFolders, string[] enabledFolders, bool enableLiveTv, bool enableLiveTvAdmin, AuthResponse authResponse, string defaultProvider, string avatarUrl)
    {
        ...
        if (avatarUrl is not null)
        {
            try
            {
                await FetchAndSaveAvatar(avatarUrl, user).ConfigureAwait(false);
            }
            catch (Exception e)
            {
                _logger.LogError(e, "Failed to fetch avatar");
            }
        }
    ...
    private async Task FetchAndSaveAvatar(string avatarUrl, User user)
    {
        // Validate URL scheme (SSO-04).
        ...
        if (avatarUri.Scheme != "https" && avatarUri.Scheme != "http")
        {
            _logger.LogWarning("Avatar URL uses disallowed scheme: {Scheme}", avatarUri.Scheme);
            return;
        }

        // Block requests to private/loopback IPs to prevent SSRF (SSO-04).
        var hostEntry = await System.Net.Dns.GetHostEntryAsync(avatarUri.Host).ConfigureAwait(false);
        foreach (var address in hostEntry.AddressList)
        {
            if (NetworkHelpers.IsPrivateOrLoopbackAddress(address))
            {
                _logger.LogWarning("Avatar URL resolves to private/loopback IP: {Address}", address);
                return;
            }
        }

        // Use a handler with disabled auto-redirects and a strict timeout (SSO-04, SSO-20).
        ...
        var avatarResponse = await client.GetAsync(avatarUri).ConfigureAwait(false);

        // Reject responses that are too large (SSO-20).
        if (avatarResponse.Content.Headers.ContentLength > 10 * 1024 * 1024)
        {
            _logger.LogWarning("Avatar response too large: {Size} bytes", avatarResponse.Content.Headers.ContentLength);
            return;
        }

        if (!avatarResponse.Content.Headers.TryGetValues("content-type", out var contentTypeList))
        {
            _logger.LogWarning("Cannot get Content-Type of avatar from: {Url}", avatarUrl);
            return;
        }

        var contentType = contentTypeList.First();

        // Validate content-type against strict allowlist (SSO-04, SSO-12).
        if (!AllowedAvatarContentTypes.Contains(contentType))
        {
            _logger.LogWarning("Avatar content type not allowed: {ContentType}", contentType);
            return;
        }

        // Map content-type to a safe file extension (SSO-12).
    ...
    private void Invalidate()
    {
        ...
        var expiredKeys = StateManager
            .Where(kvp => now.Subtract(kvp.Value.Created).TotalMinutes > 1)
            .Select(kvp => kvp.Key)
            .ToList();
...
public class TimedAuthorizeState
{
    ...
    /// <summary>
    /// Gets or sets the nonce used to bind the OIDC state to the client that initiated the flow.
    /// </summary>
