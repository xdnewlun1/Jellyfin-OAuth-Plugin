...
This plugin is under active development. PRs are welcome to improve the code.

The admin configuration page supports managing OpenID providers. SAML providers must currently be configured via the API (see [SAML Configuration](#saml) below).

**This branch targets Jellyfin 12.0 and newer.** Jellyfin 12 runs on .NET 10 and
rejects plugins built for older server ABIs, so it will not load a build made for
10.11. Use the 4.1.x releases on Jellyfin 10.11.

| Plugin version | Jellyfin server | Target framework |
| -------------- | --------------- | ---------------- |
| 5.x            | 12.0+           | net10.0          |
| 4.x            | 10.11           | net9.0           |
| 3.5.x          | 10.9 - 10.10    | net8.0           |

This works on the Web UI or clients supporting
...
This plugin validates all information passed from the client using either cryptographic signatures (SAML) or server-side state verification (OIDC). SAML responses are signature-verified, and OIDC flows are bound to the initiating client via nonce cookies. If you discover a security issue, please report it via email to <contact@ersei.net> rather than filing a public issue.

## Client Compatibility

- **Web UI**: Fully supported. This is the primary interface for SSO login.
- **Quick Connect**: Supported. Log in via the web UI first, then use Quick Connect to pair other clients.
- **Native clients** (Android, iOS, Roku, etc.): Not directly supported. The SSO auth flow requires a browser with localStorage. Use Quick Connect to authenticate native clients after an initial web login.
...
Note that the query parameter is `ApiKey`. Jellyfin 12 disables the legacy `api_key` parameter (and the `X-Emby-Token` / `X-Emby-Authorization` headers) by default.
...
- DELETE `SAML/Del/PROVIDER_NAME`: This removes a configuration for SAML for a given provider name.
- GET `SAML/Get`: Lists the configurations currently available. Sensitive fields (certificates) are redacted.
...
- POST `OID/Add/PROVIDERNAME`: This adds or overwrites a configuration for OpenID with a given provider name. It accepts JSON with the following keys and format:
  ...
  - `schemeOverride`: string. Sets the scheme for URLs used. Can be useful if the plugin refuses to use HTTPS URLs.
- DELETE `OID/Del/PROVIDER_NAME`: This removes a configuration for OpenID for a given provider name.
- GET `OID/Get`: Lists the configurations currently available. Sensitive fields (client secrets) are redacted.
- GET `OID/States`: Lists currently active OpenID flows in progress (admin only, metadata only).
...
There is no logout callback. Logging out of Jellyfin will log you out of Jellyfin only, not the SSO provider.

**This only works on the web UI**. See [Client Compatibility](#client-compatibility) above for workarounds using Quick Connect.
...
Tests cover SAML signature validation, XSS prevention, SSRF protection, and configuration serialization.
...
Anything merged to the main branch will be built and published as a nightly by our CI system.
